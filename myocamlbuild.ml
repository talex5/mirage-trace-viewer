(* OASIS_START *)
(* DO NOT EDIT (digest: 7f30c707e10cda71ce4368a9f9d91f23) *)
...
let my_dispatch conf =
  ...
  | After_rules ->
      ...
      begin match get_dir "xen-gnt", get_dir "xen-gnt-unix", get_dir "io-page", get_dir "io-page-unix" with
      | Some gnt_dir, Some gnt_unix_dir, Some io_dir, Some io_unix_dir ->
          let add_link_gnt target =
            flag ["library"; target.tag; "link_gnt"] (S [
              links ~target io_dir ["io_page"];
              links ~target io_unix_dir ["io_page_unix"];
              links ~target gnt_dir ["xen_gnt"];
              links ~target gnt_unix_dir ["xen_gnt_unix"];
            ]) in
          add_link_gnt native;
          add_link_gnt byte;
